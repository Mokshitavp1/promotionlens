  ...
  return (
    <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12, padding:28 }}>
      ...
      <p style={{ color:"var(--muted)", fontSize:13, marginBottom:24 }}>
        Bias score reduced from <span style={{ color:"var(--danger)" }}>{initialBias.toFixed(2)}</span> to <span style={{ color:"var(--accent2)" }}>{finalBias.toFixed(2)}</span> ({reduction}% improvement)
      </p>

      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:16, marginBottom:28 }}>
        {[
          { label:"Starting Score", value:initialBias.toFixed(2), color:"var(--danger)" },
          { label:"Reduction", value:`${reduction}%`, color:"var(--accent)" },
          { label:"Final Score", value:finalBias.toFixed(2), color:"var(--accent2)" },
